...
<ParamField body="recipient_wallet" type="string" required>
  Solana wallet address to receive on-chain funds.
</ParamField>
...
<ParamField body="recipient_bank_id" type="string" required>
  Bank identifier code (e.g., `gt_bank` for GuarantyCo, `access_bank` for Access Bank).
</ParamField>
...
- Wallet-type recipients must have `recipient_wallet` field
- Bank-type recipients must have `recipient_account_name`, `recipient_bank_account`, and `recipient_bank_id`
</Warning>
