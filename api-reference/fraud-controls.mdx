...
- stored fraud-score gate (`>= 75`)
- submit-time fraud re-check when `customer_wallet` is present

If submit-time fraud re-check fails internally, stored-score gating still applies.
...
2. Current submit-stage enforcement is fraud-based (not sanctions-based).
3. Some blocked responses in build/submit paths are status-only (`403`) without a structured JSON error body.
