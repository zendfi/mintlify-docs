...
## Disputes and Refunds in the Risk Lifecycle

ZendFi dispute and refund operations are integrated with existing risk controls:

- Refund creation is restricted to merchant-owned, confirmed payments.
- Partial refunds are allowed up to remaining refundable balance.
- Public dispute creation requires customer ownership checks and OTP verification.
- Dispute resolution can trigger merchant-issued refunds, preserving an auditable chain.

See [Refunds](/api-reference/refunds) and [Disputes](/api-reference/disputes) for endpoint-level behavior.
...
6. Use dispute response timelines and evidence payloads consistently for operational audits.
...
- Monitor fraud webhook streams (`FraudFlagRaised`, `FraudPaymentBlocked`) for alerting and incident response.
